...
const UserList = () => {
    ...
    const getData = async () => {
        const response = await axios.get('https://visionarybizz.com/testbackend1/user1.php');
    ...
    function daleteuser(x: UserProfile): void {
        if (window.confirm(`You want to delete the user: ${x.NameInstant}`)) {
            const url = `https://visionarybizz.com/testbackend1/user1.php?id=${x.id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then((res) => res.json())
        ...
        <MainCard
            title={
                <Grid container alignItems="center" justifyContent="space-between" spacing={gridSpacing}>
                    <Grid item>
                        <Typography variant="h3">Expired Project Lists</Typography>
                    </Grid>
                    <Grid item>
                        <SearchBar
                            value={searched}
